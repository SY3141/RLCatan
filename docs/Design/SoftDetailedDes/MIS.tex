...
\[
0 \le size \le QUEUE\_SIZE
\]
\[
head \in [0..QUEUE\_SIZE-1] \ \wedge\  tail \in [0..QUEUE\_SIZE-1]
\]
\[
isEmpty()=\top \ \Leftrightarrow\  size=0
\]
\[
isFull()=\top \ \Leftrightarrow\  size=QUEUE\_SIZE
\]

\noindent \textbf{enqueue(frame: FrameData)} (defined when $isFull()=\bot$):
\[
head' = head
\]
\[
...
\noindent \textbf{dequeue()} (defined when $isEmpty()=\bot$, output is $frame$):
\[
...
\noindent \textbf{peek()} (defined when $isEmpty()=\bot$, output is $frame$):
\[
frame = queueBuffer[head]
\]
\[
