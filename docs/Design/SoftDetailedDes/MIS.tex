...
\begin{center}
  ...
  tuple & (T1, T2, ...) & A finite, ordered list of elements, potentially of
                          different types. \\
  dictionary & dict & A collection of key-value mappings (e.g., KeyType $\rightarrow$ ValueType). \\
...
\begin{itemize}
\item \texttt{CAMERA\_RESOLUTION}: tuple of $(int, int)$ - width and height of captured frames
...
\item transition: Attempts connection to \texttt{CameraDevice}.  
      If successful: \texttt{hardwareStatus} := true.
\item output: -
...
\item exception: raises \texttt{CaptureError} if \texttt{hardwareStatus} = false  
...
\item transition:  
      \begin{itemize}
        \item If \texttt{validateMove(move)} = true,  
              apply changes encoded in $move$ to \texttt{currentGameState} and \texttt{playerAssets}.  
        \item Otherwise, no state change.
      \end{itemize}
\item output: -
\item exception: raises \texttt{InvalidMoveError} if \texttt{validateMove(move)} = false.
\end{itemize}
...
\begin{itemize}
\item transition: -
\item output: $\mathbb{B}$ - true iff $move$ is valid according to Catan rules and the  
...
\item transition:
    \begin{itemize}
        \item If \texttt{validateMove(action)} from M4 = true,  
              apply $action$ to \texttt{simulatedState} via \texttt{updateState(action)}.
        \item Apply \texttt{applyOpponentLogic()} to simulate adversarial moves.
        \item turnCount := turnCount + 1.
    \end{itemize}

\item output:  
      Returns the tuple $(\texttt{simulatedState.getState()}, \texttt{calculateReward()})$  
      of type $(GameStateData, \mathbb{R})$.

\item exception:  
      raises \texttt{StepError} if \texttt{validateMove(action)} = false.
\end{itemize}
...
\item \texttt{policyNetwork}: function mapping processed state input to  
      a probability distribution over candidate moves:
      \[
          \texttt{policyNetwork}: \texttt{ProcessedState} \rightarrow \text{dist(AIMove)}
      \]
...
\item transition:
    \begin{itemize}
        ...
        \item Evaluates result with \texttt{policyNetwork}.
...
\item transition:
    \begin{itemize}
        \item Converts $state$ to a storable format via \texttt{serializeState()}.
...
\subsubsection{Local Functions}
\begin{itemize}
\item \texttt{serializeState()}: Converts GameStateData into a storable representation.
...
\item output: frame
\item exception: QueueEmptyError if isEmpty() = True
\end{itemize}
