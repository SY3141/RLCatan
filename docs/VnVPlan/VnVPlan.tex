...
\textbf{Input/Condition:} 5--10 participants familiar with Catan complete a guided session and fill out the usability survey (Appendix~6.2). \\
...
\textbf{How test will be performed:} Peer inspection led by Matthew Cheung and Sunny Yao; issues logged in GitHub for resolution.
\end{enumerate}
...
\textbf{Output/Result:} Invalid updates are rejected; database state remains unchanged; errors logged with descriptive messages. \\
\textbf{Test Case Derivation:} Verifies validation portion of NFR.S.5, preventing data corruption from malformed inputs. \\
\textbf{How test will be performed:} Automated tests inject invalid game state objects and verify the database rejects them and remains consistent.
\end{enumerate}
...
\subsection{Availability (NFR.S.6)}
