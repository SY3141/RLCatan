from typing import Iterable

from catanatron import Game, Action, Player
from catanatron.cli import register_cli_player

import random
from catanatron.state_functions import (
    player_key,
)


class PlacementPlayer(Player):

    def read_map(self, game: Game, player_color: str) -> Action:
        """Called once at the start of the game to read the map.
        Args:
            game (Game): complete game state. read-only.
        """
        placement_actions = game.playable_actions(player_color)
        if len(placement_actions) == 1:
            return placement_actions[0]
        for i, action in enumerate(placement_actions):
            print(f"[{i}] {action}")

    def decide(self, game: Game, playable_actions):
        if game.state.turn == 0:
            return self.read_map(game, self.color)
        
        if len(playable_actions) == 1:
            return playable_actions[0]

        best_value = float("-inf")
        best_actions = []
        for action in playable_actions:
            game_copy = game.copy()
            game_copy.execute(action)

            key = player_key(game_copy.state, self.color)
            value = game_copy.state.player_state[f"{key}_ACTUAL_VICTORY_POINTS"]
            if value == best_value:
                best_actions.append(action)
            if value > best_value:
                best_value = value
                best_actions = [action]

        return random.choice(best_actions)


register_cli_player("PP", PlacementPlayer)
