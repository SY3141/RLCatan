...
class RewardWrapper(gym.Wrapper):
    ...
    def get_player_production_pips(self, game, player_color) -> Dict[str, int]:
        production = {res: 0 for res in TYPICAL}

        # Board buildings: { node_id: (color, building_type) }
        # We need to access the map structure to find tiles.
        board = game.state.board

        for node_id, (owner_color, building_type) in board.buildings.items():
            if owner_color != player_color:
                continue

            multiplier = 2 if building_type == "CITY" else 1

            # adjacent_tiles is { node_id: [LandTile, ...] }
            adjacent_tiles = board.map.adjacent_tiles.get(node_id, [])

            for tile in adjacent_tiles:
                if tile.resource is None or tile.number is None:
                    continue

                res_name = tile.resource
                if res_name in production:
                    pips = self.get_pips_for_number(tile.number)
                    production[res_name] += pips * multiplier

        return production

    def get_pips_for_number(self, number: int) -> int:
        """Maps a dice roll number (2-12) to its pip count (1-5)."""
        return {
            2: 1, 12: 1,
            3: 2, 11: 2,
            4: 3, 10: 3,
    ...
    def reset(self, **kwargs):
        ...
        # Reset production pips tracking
        game = cast(CatanatronEnv, self.env.unwrapped).game
        active_color = self._active_color()
        if active_color is not None:
             pips_dict = self.get_player_production_pips(game, active_color)
             self.last_production_pips = sum(pips_dict.values())
        else:
             self.last_production_pips = 0
    ...
    def step(self, action):
        ...
        Execute one environment step, then compute shaped reward components:
          ...
          5. prod_reward: scaled positive delta in total production pips.
          6. decay: exponential decay applied to aggregate (base + shaping).
        ...
        # Production pips tracking
        game = cast(CatanatronEnv, self.env.unwrapped).game
        active_color = self._active_color()
        production_pips_total = 0
        prod_reward = 0.0
        
        if active_color is not None:
            production_pips = self.get_player_production_pips(game, active_color)
            production_pips_total = sum(production_pips.values())
            
            # Calculate production increase reward
            pip_delta = production_pips_total - self.last_production_pips
            if pip_delta > 0:
                prod_reward = self.production_scale * pip_delta
                print(
                        f"[RewardWrapper] Production increased by {pip_delta} pips "
                        f"(total: {production_pips_total}). Reward: {prod_reward:.3f}",
                        flush=True,
                    )

            # Update last known pips
            self.last_production_pips = production_pips_total
            
            # Log pip details
            info["production_pips_total"] = production_pips_total
            for res, pips in production_pips.items():
                info[f"production_pips_{res}"] = pips

        # Aggregate shaping and decayed final reward.
        shaping_total = gain_reward + spend_reward + build_bonus + prod_reward
        ...
        info["reward_shaping_production_increase"] = prod_reward
